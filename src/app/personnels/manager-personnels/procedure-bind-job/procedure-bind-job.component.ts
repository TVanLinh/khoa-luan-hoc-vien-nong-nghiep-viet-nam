...
export class ProcedureBindJobComponent extends BaseFormComponent implements OnInit {
  ...
  onProcess($event) {
    if ($event._id) {
      this.taskService.put(Config.BIND_JOB_URL, {data: $event}).subscribe((data) => {
      ...
      this.taskService.post(Config.BIND_JOB_URL, {data: $event}).subscribe((data) => {
  ...
  removeHandler($event) {
    this.taskService.delete2(Config.BIND_JOB_URL, {data: $event}).subscribe(data => {
